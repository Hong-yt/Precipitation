...
class ShapAnalyzer:
    ...
    def create_explainer(self, background_data, batch_size=None):
        """
        创建SHAP解释器，使用改进的KernelExplainer来解决样本数小于特征数的问题

        参数:
        background_data: 用于建立解释器期望值的背景数据
        batch_size: 批次大小（仅为兼容性保留，不使用）
        """
        print("创建改进的KernelExplainer...")
    ...
    def compute_shap_values(self, X, batch_size=None, nsamples=None):
        """
        计算测试集样本的SHAP值，使用多种策略解决样本数小于特征数的问题
        ...
        # 设置合适的nsamples参数
        if nsamples is None:
            # 根据特征数和样本数动态调整nsamples
            n_features = X.shape[1]
            n_samples = X.shape[0]

            # 确保nsamples足够大，至少是特征数的2倍
            min_nsamples = max(n_features * 2, 500)
            default_nsamples = getattr(Config, 'SHAP_NSAMPLES', 200)
            nsamples = max(min_nsamples, default_nsamples)

        print(f"使用{nsamples}个样本进行SHAP估计（特征数: {X.shape[1]}）")

        # 尝试多种策略计算SHAP值
        strategies = [
            {"nsamples": nsamples, "l1_reg": "auto"},
            {"nsamples": nsamples * 2, "l1_reg": "auto"},
            {"nsamples": max(X.shape[1] * 3, 1000), "l1_reg": "aic"},
            {"nsamples": max(X.shape[1] * 5, 2000), "l1_reg": "bic"},
        ]

        for i, strategy in enumerate(strategies):
            try:
                print(
                    f"尝试策略 {i + 1}/{len(strategies)}: nsamples={strategy['nsamples']}, l1_reg={strategy['l1_reg']}")

                # 使用KernelExplainer计算SHAP值
                shap_values = self.explainer.shap_values(
                    X,
                    nsamples=strategy['nsamples'],
                    l1_reg=strategy['l1_reg']
                )
                ...
                if shap_values is None or len(shap_values) == 0:
                    print(f"策略 {i + 1} 未能计算得到有效的SHAP值")
                    continue

                print(f"策略 {i + 1} 成功计算SHAP值，形状: {shap_values.shape}")
                # 将SHAP值保存到实例变量中
        ...
        # 如果所有策略都失败，尝试分批计算
        print("所有策略都失败，尝试分批计算SHAP值...")
        try:
            return self._compute_shap_values_in_small_batches(X, nsamples)
        except Exception as batch_e:
            print(f"分批计算也失败: {str(batch_e)}")

            # 最后的备选方案：逐个样本计算
            print("尝试逐个样本计算SHAP值...")
            return self._compute_shap_values_one_by_one_safe(X, nsamples)

    def _compute_shap_values_in_small_batches(self, X, nsamples):
        """
        使用小批次计算SHAP值，每次只处理少量样本
        """
        print("使用小批次策略计算SHAP值...")

        # 使用更小的批次大小
        batch_size = min(10, len(X) // 4)
        if batch_size < 1:
            batch_size = 1
        ...
        for i in tqdm(range(n_batches), desc="小批次计算SHAP值"):
            ...
            try:
                # 为小批次使用更大的nsamples
                batch_nsamples = max(nsamples, X.shape[1] * 3)
                batch_shap = self.explainer.shap_values(
                    batch_data,
                    nsamples=batch_nsamples,
                    l1_reg="auto"
                )
                ...
                print(f"批次 {i + 1} 计算失败: {e}")
                # 对失败的批次使用零值
                zero_batch = np.zeros((len(batch_data), X.shape[1]))
                all_shap_values.append(zero_batch)

        # 合并所有批次
        combined_shap = np.vstack(all_shap_values)
        print(f"小批次计算完成，SHAP值形状: {combined_shap.shape}")
        self.shap_values = combined_shap
        return combined_shap

    def _compute_shap_values_one_by_one_safe(self, X, nsamples):
        """
        安全的逐个样本计算SHAP值，使用更保守的参数
        """
        print("使用安全的逐个样本策略...")
        all_shap_values = []

        # 为单样本计算使用更大的nsamples
        single_nsamples = max(nsamples, X.shape[1] * 5)

        for i in tqdm(range(len(X)), desc="安全单样本计算"):
            try:
                sample = X[i:i + 1]
                sample_shap = self.explainer.shap_values(
                    sample,
                    nsamples=single_nsamples,
                    l1_reg="auto"
                )
                ...
                print(f"样本 {i} 计算失败: {e}")
                # 使用零值替代
                zero_sample = np.zeros((1, X.shape[1]))
                all_shap_values.append(zero_sample)

        # 合并所有样本
        combined_shap = np.vstack(all_shap_values)
        print(f"安全单样本计算完成，SHAP值形状: {combined_shap.shape}")
        self.shap_values = combined_shap
        return combined_shap
    ...
    def save_shap_values(self, output_dir, dataset_name, data=None, fold_idx=None):
        ...
        if self.feature_names is not None:
            ...
            plt.barh(importance_df['feature'].values[:23], importance_df['importance'].values[:23])
            plt.xlabel('平均|SHAP值|')
            plt.title('SHAP特征重要性 (前23)')
            plt.tight_layout()
