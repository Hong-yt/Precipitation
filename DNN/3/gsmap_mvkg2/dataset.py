...
class PrecipitationDataProcessor:
    def __init__(self, dataset_name, features_path, target_path):
        ...
        self.original_sample_count = None  # 添加原始样本数量属性
    ...
    def load_data(self):
        ...
        # 记录原始样本数量
        self.original_sample_count = len(self.features_df)
    ...
    def get_augmentation_flags(self, indices):
        """
        获取样本是否为增强样本的标志
        ...
        Returns:
            numpy.ndarray: 布尔数组，True表示增强样本，False表示原始样本
        """
        if self.original_sample_count is None:
            raise ValueError("请先调用load_data()方法加载数据")

        # 如果索引小于原始样本数量，则为原始样本
        return np.array([idx >= self.original_sample_count for idx in indices])
