    ...
    for idx, station in tqdm(stations.iterrows(), total=len(stations), desc="处理站点"):
        ...
        with rasterio.open(auxiliary_1km_path) as auxiliary_1km_src:
            ...
            if (
                ...
                print(f"站点{station_id}没有有效的auxiliary数据或有效单元格太少。跳过处理。")
                continue
            ...
            if len(valid_auxiliary_values) > 0:
                ...
                if len(station_index) > 0:
                    ...
                    print(f"在网格单元数据中找不到站点{station_id}的auxiliary值。")
            else:
...
if __name__ == "__main__":
    auxiliary_025_path = "D:/hongyouting/some_preprocess/dnn/auxiliary/025/AI/AI_1440400.tif"  # 0.25°auxiliary文件
    auxiliary_1km_path = "D:/hongyouting/some_preprocess/dnn/auxiliary/1km/1km_world/1440400_AI.tif"  # 1km auxiliary文件
    station_csv_path = "D:/hongyouting/some_preprocess/station/stations_summary.csv"  # 站点数据
    output_csv_path = "D:/hongyouting/some_preprocess/dnn/rep/AI_representativeness_results.csv"  # 输出文件
    detailed_output_path = "D:/hongyouting/some_preprocess/dnn/rep/AI_representativeness_detailed.csv"  # 详细输出文件
