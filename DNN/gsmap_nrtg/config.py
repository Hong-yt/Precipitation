...
class Config:
    # 数据集配置
    DATASET_NAME = "gsmap_nrtg"
    FEATURES_PATH = f"D:/data/dnn/merge/features/1/{DATASET_NAME}_features.xlsx"
    TARGET_PATH = f"D:/data/dnn/merge/target/20464_target.xlsx"

    # 输出目录配置
    OUTPUT_DIR = 'D:/result/dnn/3/gsmap_nrtg'
    ...
    SHAP_OUTPUT_DIR = os.path.join(OUTPUT_DIR, 'shap')  # SHAP分析结果目录
    ...
    # 特征工程配置
    FEATURE_ENGINEERING = False  # 是否应用特征工程
    NONLINEAR_TRANSFORM = False  # 是否应用非线性变换（sqrt、square、log等）
    INTERACTION_FEATURES = False  # 是否创建交互特征
    INTERACTION_DEGREE = 2  # 交互特征的多项式次数
    STATISTICAL_FEATURES = False  # 是否创建统计特征（均值、标准差等）
    ...
    # SHAP分析配置
    SHAP_ANALYSIS = True  # 是否进行SHAP分析
    SHAP_BACKGROUND_SAMPLES = 200  # 背景数据集的样本数
    SHAP_NSAMPLES = 200  # KernelExplainer的nsamples参数，决定SHAP估计的精度
    SHAP_BATCH_SIZE = 1  # SHAP分析的批次大小
    SHAP_TOP_FEATURES = 23  # 显示的最重要特征数量
    SHAP_FORCE_PLOT_SAMPLES = 10  # 为多少个样本创建力图
    SHAP_FORCE_PLOT_ENABLED = False  # 是否生成单个样本的力图（建议关闭，只关注整体SHAP分析）
    ...
    MIN_BATCH_SIZE = 2  # 最小批次大小，避免BatchNorm问题
    ...
    # 模型配置
    HIDDEN_SIZES = [256, 128, 64, 32]  # 调整为更复杂的网络
    DROPOUT_RATE = 0.2
    ...
    def create_directories(cls):
        ...
        os.makedirs(cls.SHAP_OUTPUT_DIR, exist_ok=True)  # 创建SHAP分析结果目录
