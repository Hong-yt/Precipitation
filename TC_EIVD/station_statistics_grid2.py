    ...
    for (lat_idx, lon_idx), date_data in grid_data.items():
        ...
        for date, precip_list in date_data.items():
            ...
            if station_date_idx >= satellite_data.shape[0]:
                print(f"Date out of range for station: on day {station_date_idx}")            #     continue
        ...
        # 再次打印它们的形状
        # print(
        #     f"Before calculate - station_precip_values: {station_precip_values.shape}, satellite_precip_vals: {satellite_precip_vals.shape}")
...
if __name__ == '__main__':
    ...
    output_file = 'D:/hongyouting/result/station_statistics/1/station_gsmap_mvkg.csv'
